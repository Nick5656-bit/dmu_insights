...
export function SendSurveyWizard({ templates, clubs, createBatchAction }: SendSurveyWizardProps) {
  ...
  return (
    <div className="min-w-0 space-y-6 [&>section]:p-4 sm:[&>section]:p-6">
      ...
      {step === 3 && !isAnnual ? (
        <section className="rounded-[28px] border border-border/70 bg-card p-6 shadow-sm">
          ...
          <div className="mt-5 flex flex-wrap items-end gap-3 rounded-2xl border bg-muted/15 p-4">
            <label className="min-w-0 basis-full text-sm font-medium sm:min-w-[260px] sm:flex-1">Fælles sendetidspunkt<input className="mt-1" type="datetime-local" value={sharedSendAt} onChange={(input) => setSharedSendAt(input.target.value)} /></label>
          ...
          <div className="mt-3 flex flex-wrap items-end gap-3 rounded-2xl border bg-muted/15 p-4">
            <label className="min-w-0 basis-full text-sm font-medium sm:min-w-[260px] sm:flex-1">Fælles lukketidspunkt<input className="mt-1" type="datetime-local" value={sharedClosesAt} onChange={(input) => setSharedClosesAt(input.target.value)} /></label>
