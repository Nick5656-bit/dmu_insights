    ...
    {rows.length ? <>
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <div className="inline-flex rounded-full border border-border p-1 text-xs" role="group" aria-label="Grafvisning">
          <button type="button" aria-pressed={!showTimeline} onClick={() => setView("comparison")} className={`min-h-11 rounded-full px-3 py-2 sm:min-h-0 ${!showTimeline ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Sammenligning</button>
          <button type="button" aria-pressed={showTimeline} disabled={timeline.length < 2} onClick={() => setView("timeline")} className={`min-h-11 rounded-full px-3 py-2 disabled:opacity-50 sm:min-h-0 ${showTimeline ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Udvikling over tid</button>
        </div>
      ...
      <div className={`mt-4 hidden min-w-0 sm:block ${scrollCategories ? "overflow-x-auto" : "overflow-x-hidden"}`} role="region" aria-label="Kategorier og samlet gennemsnit" tabIndex={scrollCategories ? 0 : undefined}>
      ...
      <MobileResultBars rows={rows} means={means} color={color} />
      </>}
      ...
      <details className="mt-3 text-xs text-muted-foreground">
        ...
        {showTimeline && <>
          ...
          <div className="overflow-x-auto"><table className="mt-3 w-full text-left"><caption className="text-left">Udvikling: {selectedCategory || "Samlet gennemsnit"}</caption>
